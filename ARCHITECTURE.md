...
- **The synthetic drag's grab point is layout-visible.** Ice grabbed at `(20_000, 20_000)`; that x clamps to the right end of the menu bar, and the window server opens an eased make-room gap there — every on-screen item lurches left and springs back on each programmatic move. The drag therefore starts at the item's own (off-screen) location, where any make-room is invisible.
- **The reveal's full item-width drop offset is load-bearing, not cosmetic.** Any drop closer to the icon (tested at 0.9×, 0.75×, 0.5×, 0.1×, and 0× the item width) makes the window server insert the item on the icon's *right* instead of its left.
