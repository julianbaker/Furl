...
| **Auto-hide after use** | Once revealed, the item hides when the auto-hide interval elapses with no menu open and the pointer off the item, or (if the user opened its menu) once that menu is dismissed. Then a Command-drag slides it back off-screen — deferred until no mouse button is down, so the move never interrupts a real click or drag. | Accessibility (for the move) | `ControlItem.swift` |
| **Position reconciliation** | After each periodic enumeration refresh (~7 s), items that drifted from the user's configured intent (apps restore their own saved positions at launch) are moved back: visible managed items are tucked off-screen; excluded items are returned beside the icon. Uses the same Command-drag mechanism; rate-limited per item, paused during peeks or while Furl's own menu is open, and deferred while the user is mid-click or mid-drag. This is the one place synthetic events are posted *without* a same-moment user action — they enforce positions the user configured in settings. | Accessibility (for the moves) | `ControlItem.swift` |
...
- **NSEvent monitors** — **none**, global or local. (Pointer *state* — `NSEvent.mouseLocation` / `NSEvent.pressedMouseButtons` — is polled to hold auto-hide while the pointer is on a revealed item and to defer moves during a real click or drag; polling reads mouse state only, installs nothing, and touches no keyboard input.)
...
- **User-initiated relaunch helper** — the dropdown's *Relaunch Furl* item (offered when the Accessibility state looks stuck) spawns a one-shot `/bin/sh -c "sleep 0.5; open <own bundle>"` and terminates. It runs only from that explicit menu action, relaunches only Furl itself, and leaves nothing resident (`ControlItem.swift`).
